...
def financial_article_cleaner(file_name):
    ...
    article_sentiments_company["body_text"] = (
        article_sentiments_company["body_text"].astype(str) + "---newarticle---"
    )
    company_bodytext = article_sentiments_company["body_text"]
    pd.set_option("display.max_colwidth", -1)

    with open("company_bodytext.txt", "w", encoding="utf-8") as f:
        f.write(company_bodytext.to_string(header=False, index=False))
...
def financial_article_cleaner_v2(file_name, ticker, start_date, end_date):
    ...
    news_df_new.drop_duplicates(subset="title", keep="first", inplace=True)
    ...
    news_df_combined["news_combined"] = news_df_combined.groupby(["publish_date"])[
        "body_text"
    ].transform(lambda x: " ".join(x))
    news_df_combined.drop_duplicates(subset="publish_date", keep="first", inplace=True)
    ...
    merge = stock_df.merge(
        news_df_combined, how="inner", left_index=True, right_index=True
    )
    ...
    stock_df_label["Label"] = stock_df_label.apply(
        lambda x: 1 if (x["Adj Close Next"] >= x["Adj Close"]) else 0, axis=1
    )
    ...
    merge2 = stock_df.merge(
        stock_df_label_adj_nxt, how="inner", left_index=True, right_index=True
    )
    merge2 = merge2.dropna()

    merge3 = stock_df_label_adj_nxt.merge(
        merge, how="inner", left_index=True, right_index=True
    )
...
def bert_data_tokenizer(article_sentiments, tokenizer):
    ...
    filtered_articles = [
        [word for word in article if not word in stop_words if word.isalpha()]
        for article in tokens
    ]
    filtered_articles_joined = [
        ",".join(article).replace(",", " ") for article in filtered_articles
    ]
